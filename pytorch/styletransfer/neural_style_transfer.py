...
def imsave(tensor, path='./images/out.jpg'):
    image = tensor.cpu().clone()  # we clone the tensor to not do changes on it
    image = image.squeeze(0)      # remove the fake batch dimension
    image = unloader(image)
    image.save(path)
...
def main(inputimg="./images/dancing.jpg", styleimg="./images/picasso.jpg", numsteps=50):
    ...
    content_img = image_loader(inputimg)

    assert style_img.size() == content_img.size(), \
        "we need to import style and content images of the same size" + str(style_img.size()) + " != " + str(content_img.size())
...
if __name__ == "__main__":
    ...
    required_args.add_argument("--input", help="Path to input image", required=True)
    required_args.add_argument("--style", help="Path to style image", required=True)
    parser.add_argument("--steps", help="Number of iterations")
    args = parser.parse_args()
    steps = 50
    if args.steps:
        steps = int(args.steps)
    main(args.input, args.style, steps)
